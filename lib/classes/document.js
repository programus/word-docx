...
class WordDocument {
   ...
   * @property {number} countIndex - the index of the node
   */
  ...
  findRunNodePair(nodeInfo, referPair) {
    const info = nodeInfo.node.info
    if (info && referPair && referPair[0]) {
      // because a new node added before while splitting the original node
      info.countIndex++
    }
   ...
   * @param {NodeInfo} nodeInfo - an Object in format
