...
class WordDocument {
  ...
  setCssClass(className, formatJson) {
    const rf = new RunFormat(formatJson, true)
    this.classes[className] = rf.styles
    if (this.owner) {
      this.owner.dispatchEvent(new CustomEvent('css-class-updated', {
        ...
        detail: {
          className, 
          cssTexts: rf.styles.map(style => style.cssText),
        },
      }))
    }
  }
  ...
  applyFormat(node, format) {
    ...
    prNode.$cssClasses = format.cssClasses.map(className => this.classes[className].map((_, i) => `${className}-${i}`)).flat()
    let styles = (format.cssClasses || []).map(c => this.classes[c]).flat()
    if (format.styles) {
  ...
  /**
   * Find the node by using the provided html node
   * @param {HTMLElement} htmlNode - the html node
   * @returns {DocNode|undefined} - the run node or undefined
   */
  findDocNodeFromHtmlNode(htmlNode) {
    return htmlNode && (htmlNode.$node || this.findDocNodeFromHtmlNode(htmlNode.parentElement))
  }
  ...
  pickupRunNodesFromSelection(selection) {
    ...
    if (selection.toString().length > 0) {
      ...
      if (positionComparison & Node.DOCUMENT_POSITION_FOLLOWING) {
        ...
        fromNode = this.findDocNodeFromHtmlNode(selection.anchorNode)
        toOffset = selection.focusOffset
        toNode= this.findDocNodeFromHtmlNode(selection.focusNode)
      } else if (positionComparison & Node.DOCUMENT_POSITION_PRECEDING) {
        fromOffset = selection.focusOffset
        fromNode = this.findDocNodeFromHtmlNode(selection.focusNode)
        toOffset = selection.anchorOffset
        toNode = this.findDocNodeFromHtmlNode(selection.anchorNode)
      } else {
