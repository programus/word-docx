...
class WordDocx extends HTMLElement {
  ...
  _generateStyle() {
    ...
    this.addEventListener('css-class-updated', (evt) => {
      const styleSheetText = Object.entries(this.wordDoc.classes).map(([className, styles]) => styles.map((style, i) => `#_word-docx-root[enable-css='true'] .${className}-${i} {${style.cssText}}`).join('\n')).join('\n')
